...
const CoachPage: React.FC = () => {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            if (openAIKey && keyStatus === 'valid') {
                console.log("Using OpenAI API for Sensei suggestion.");
                ...
                console.log("Using Gemini API for Sensei suggestion.");
    ...
    return (
        <div className="space-y-6">
            <h2 className="text-2xl font-bold text-center" style={{fontFamily: "'Rajdhani', sans-serif"}}>Parle au Sensei</h2>
            ...
            <div className="space-y-4">
                {suggestions.map((program, index) => (
                    <Card key={index}>
                        ...
                        <p className="text-sm text-dojo-text-muted">{program.category?.join(', ')} - {program.estDurationMin || 'N/A'} min</p>
                        <ul className="mt-2 list-disc list-inside text-sm">
                            {(program.exercises || []).map((ex: any, i) => (
                                <li key={i}>{ex.name} ({ex.target?.reps ? `${ex.target.reps} reps` : `${ex.target?.timeSec} sec`})</li>
                            ))}
                        </ul>
                    </Card>
                ))}
